...
const AIChat = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const msg = data.error || (getBrowserLanguage() === 'pt'
          ? 'Chat não disponível no momento. Use o formulário de contato.'
          : getBrowserLanguage() === 'es'
            ? 'Chat no disponible. Use el formulario de contacto.'
            : 'Chat is not available. Please use the contact form.')
  ...
  return (
    <>
      ...
      {/* Chat Window - top abaixo do header (faixa contato + nav h-20 ≈ 8rem) */}
      {isOpen && (
        <div
          className="fixed z-[9998] bg-white shadow-2xl flex flex-col overflow-hidden border border-gray-200 inset-0 pt-24 sm:pt-0 sm:inset-auto sm:top-[8rem] sm:bottom-24 sm:right-6 sm:left-auto sm:rounded-2xl sm:w-full sm:max-w-md"
          style={{
